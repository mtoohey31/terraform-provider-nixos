...
func main() {
	...
	case "build --out-link .terraform-provider-nixos/test-machine-id path/to/other/flake#nixosConfigurations.other.flake.attr.config.system.build.toplevel":
		...
		if err := os.Symlink("/nix/store/other-profile-path", ".terraform-provider-nixos/test-machine-id"); err != nil {
			panic(err)
		}

	case "copy --to ssh://test-username@127.0.0.1 /nix/store/test-profile-path",
		"copy --to ssh://test-username@127.0.0.1 /nix/store/other-profile-path":
  ...
  "/nix/store/other-profile-derivation.drv": {
    "outputs": {
      "out": {
        "path": "/nix/store/other-profile-path"
      }
    }
  }
}`)
